...
export class ProdutoDAO extends GenericDAO {
  constructor() {
    super(driver, 'produtos');
    this.initializeProdutos();
  }

  async initializeProdutos() {
    try {
      const produtos = await driver.all("produtos");
      if (!produtos || produtos.length === 0) {
        ...
        for (const produto of produtosMock) {
          await driver.save("produtos", produto);
        }
      }
    } catch (error) {
      throw new Error(`Erro ao inicializar produtos: ${error.message}`);
    }
  }
}
